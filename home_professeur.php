...
if (isset($_POST['ajouter_module'])) {
    if (!empty($_POST['titre']) && !empty($_POST['Code_Cours'])) {
        ...
        $sql = "INSERT INTO module(IdParent, titre, presentation, mots_cles, Code_Cours, cible, prerequis, est_progressif, proprietaire)
                VALUES($IdParent, '$titre', '$presentation', '$mots_cles', '$code_cours', '$cible', '$prerequis', $est_progressif, $proprietaire)";
  ...
  //get all modules from db
    ...
    if (isset($_POST['ajouter_cousre_item'])) {
        if(!empty($_FILES['contenu']['name'])){
          ...
          $select_parent="SELECT * FROM module WHERE titre='$module_nom' AND proprietaire=$userID";
          $query=mysqli_query($conn,$select_parent);
          $row=mysqli_fetch_object($query);
          $ParentID=$row->IdParent;
          $est_accessible=$_POST['accessible'];
     ...
     <div class="add-course-item">
        <div class="sub-title">
         ...
         <form method="POST" class="myform" enctype="multipart/form-data">
             ...
             <div class="input-pass">
                <select class='box' id="accessible" name="accessible">
        ...
        <tbody>
         <?php foreach ($cours as $cour ) { ?>
              <tr>
                   ...
                   <td><a href="afficher_chapitres.php?moduleID=<?php echo $cour['IdParent'] ?>"><div class="table_icon modify"><i class="ri-eye-fill"></i></div></a></td>
                   <td>
                    <form  method="post">
